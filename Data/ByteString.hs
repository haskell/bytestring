...
foldl f z (PS fp off len) =
    go (end `plusPtr` len)
  where
    end = (unsafeForeignPtrToPtr fp) `plusPtr` (off-1)
...
foldr :: (Word8 -> a -> a) -> a -> ByteString -> a
foldr k z (PS fp off len) = go ptr
  where
    ptr = unsafeForeignPtrToPtr fp `plusPtr` off
    end = ptr `plusPtr` len
...
any f (PS x s len) = accursedUnutterablePerformIO $ withForeignPtr x g
  where
    g ptr = go ptr'
      where
        ptr' = ptr `plusPtr` s
...
all :: (Word8 -> Bool) -> ByteString -> Bool
all _ (PS _ _ 0)   = True
all f (PS x s len) = accursedUnutterablePerformIO $ withForeignPtr x g
  where
    g ptr =  go ptr'
      where
        ptr' = ptr `plusPtr` s
...
splitWith pred_ (PS fp off len) = splitWith0 off len fp
...
--
elemIndexEnd :: Word8 -> ByteString -> Maybe Int
elemIndexEnd ch (PS x s len) = accursedUnutterablePerformIO $ withForeignPtr x g
  where
    g ptr = go (len-1)
      where
        p = ptr `plusPtr` s
...
-- predicate.
filter :: (Word8 -> Bool) -> ByteString -> ByteString
filter k ps@(PS x s len)
    | null ps   = ps
    | otherwise = unsafePerformIO $ createAndTrim len $ \p -> withForeignPtr x $ \f -> do
        t <- go' f p
