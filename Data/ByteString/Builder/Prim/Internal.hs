...
withSizeFB feSize (BP b io) =
    BP (lSize + b)
       (\x op0 -> do let !op1 = op0 `plusPtr` lSize
                     ...
                     return op2)
  where
    FP lSize ioSize = feSize (fromIntegral b)


{-# INLINE withSizeBB #-}
withSizeBB :: BoundedPrim Word -> BoundedPrim a -> BoundedPrim a
withSizeBB (BP bSize ioSize) (BP b io) =
    BP (bSize + 2*b)
...
liftIOB (BP l io) = BP l (\xWrapped op -> do x <- xWrapped; io x op)
-}
